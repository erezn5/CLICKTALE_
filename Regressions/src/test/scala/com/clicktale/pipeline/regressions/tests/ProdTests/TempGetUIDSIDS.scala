...
class TempGetUIDSIDS extends WordSpecLike with BeforeAndAfterAll {
  "GetFromVertica Prod US" taggedAs (Tag.apply("US")) in {
    val aaaaa = File.ReadAllText("c:\\aaa\\shahaf.csv").split("\n")
    var pid: String = ""
    var subsId: String = ""
    aaaaa.foreach {
      ent =>
        if (!(ent.split(',')(0).contains("SubsId"))) {
          try {
            ...
            if (!File.Exists(s"c:\\aaa\\results\\US\\new\\$subsId-$pid.csv")) {
              ...
              val count = Vertica.selectCount(s"select count(*) from CT1Analytics.Recordings where CreateDate>'$from' and CreateDate<'$to' and SubscriberID=$subsId and pid=$pid")
              ...
              if (count.toInt < 50000) {
                ...
                File.AppendAllText(s"c:\\aaa\\results\\US\\new\\$subsId-$pid.csv", res)
              }
              else
